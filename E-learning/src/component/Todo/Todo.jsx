...
function Todo() {
  ...
  const fetchTodos = async () => {
    const url = `http://localhost:3000/todo/get?userId=${user._id}&todo=${JSON.stringify(todoData)}`;
    try {
  ...
  const showTodos = () => {
    ...
    return filteredTodos.map(todo => (
      <li key={todo._id}>
      {todo.content}
      <button onClick={() => updateTodoStatus(todo._id, todo.status === 'Complete' ? 'Incomplete' : 'Complete')}>
  ...
  return (
    <div>
      ...
      <div className="todos-container">
        {todos.length === 0 && (
          <img className="empty-image" src="empty.svg" alt="Empty list" />
        )}
      </div>
    </div>
  );
